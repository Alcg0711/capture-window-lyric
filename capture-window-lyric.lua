...
BOOL GetVersionExW(void* lpVersionInformation);
typedef struct {
...
local function detect_os_version()
    local os_info = ffi.new("OSVERSIONINFOEXW")
    os_info.dwOSVersionInfoSize = ffi.sizeof("OSVERSIONINFOEXW")
    
    if kernel32.GetVersionExW(ffi.cast("void*", os_info)) then
        local major = tonumber(os_info.dwMajorVersion)
        local build = tonumber(os_info.dwBuildNumber)
        
        if major == 10 then
            os_version = build >= 22000 and "win11" or "win10"
        else
            os_version = "win10"
        end
    else
    ...
    log_info("检测到系统版本：" .. os_version .. " (build: " .. tonumber(os_info.dwBuildNumber) .. ")")
end
...
local function GetLong(hwnd,i)
    return is_64bit and tonumber(user32.GetWindowLongPtrW(hwnd,i)) or tonumber(user32.GetWindowLongW(hwnd,i))
end

local function SetLong(hwnd,i,v)
    if is_64bit then
        user32.SetWindowLongPtrW(hwnd,i,ffi.cast("LONG_PTR",v))
    else
        user32.SetWindowLongW(hwnd,i,ffi.cast("LONG",v))
    end
end
...
local function set_taskbar_visible(hwnd, safe_parent)
    ...
    if os_version == "win11" then
        ...
        ex = bit.band(ex, bit.bnot(WIN.WS_EX_TOOLWINDOW))
        ex = bit.bor(ex, WIN.WS_EX_APPWINDOW)
    end
...
local function match_pattern_rule(pname, cls, title, pattern)
    ...
    return wildcard_match(title, title_pat)
end
...
function script_properties()
    ...
    obs.obs_properties_add_button(p,"run","运行",function() 
        local ok, err = pcall(run_detection)
        if not ok then log_info("手动检测异常: " .. tostring(err)) end
        return true 
    end)

    obs.obs_properties_add_button(p,"print_windows_info","输出窗口信息",function()
        local ok, err = pcall(print_windows_info)
        if not ok then log_info("输出窗口信息异常: " .. tostring(err)) end
        return true
    end)
...
v <a href="https://github.com/Alcg0711/capture-window-lyric">0.1</a>
